...
// --- Upload a Cloudinary + guardar Evidencia ---
app.post('/api/upload', requireAuth, upload.single('file'), async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'file requerido' })

    const folder = req.body.folder || 'amcode/attendance'
    const kind   = req.body.kind === 'RECORDING' ? 'RECORDING' : 'SCREENSHOT'
    const note   = req.body.note
    const marcId = req.body.marcacionId ? Number(req.body.marcacionId) : null

    cloudinary.uploader.upload_stream(
      { folder, resource_type: 'auto' },
      async (err, result) => {
        if (err) return res.status(500).json({ error: 'Cloudinary error', details: err.message })

        // Validar pertenencia de marcación (si vino)
        let marcacionId = null
        if (marcId) {
          const owns = await prisma.marcacion.findFirst({
            where: { id: marcId, usuarioId: req.user.id }
          })
          if (owns) marcacionId = owns.id
        }
...
// --- Evidencias: listados ---
app.get('/api/evidencias/mias', requireAuth, async (req, res) => {
  const rows = await prisma.evidencia.findMany({
...
app.get('/api/evidencias', requireAuth, requireRole('ADMIN','MANAGER'), async (req, res) => {
